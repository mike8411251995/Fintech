...
def csv_to_dataset(csv_path):
    ...
    data = data.drop('trading_point', axis=1)
    # data = data.drop(0, axis=0)

    data = data.values
    new_data = np.zeros_like(data)
    new_data = new_data[:-1]
    new_data[:, :3] = data[:-1, 1:4]
    new_data[:, 4] = data[:-1, 4]
    new_data[:, 3] = data[1:, 0]
    data = new_data
    ...
    return ohlcv_histories_normalised, technical_indicators_normalised, next_day_open_values_normalised, next_day_open_values, next_day_open_higher, y_normaliser
