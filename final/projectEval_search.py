...
import numpy as np
import pandas as pd
from myStrategy_search import myStrategy
...
for beta in tqdm(range(0, 100)):
    for alpha in tqdm(range(0, 100)):
        for windowSize in tqdm(range(300)):
            params = (windowSize, alpha, beta)
            # dailyOhlcv = pd.read_csv('final/TX_daily.csv')
            # minutelyOhlcv = pd.read_csv('final/TX_minutely.csv')
            capital = 500000.0
            capitalOrig=capital
            transFee = 100
            evalDays = 14
            ...
            for ic in range(evalDays,0,-1):
                ...
                for i in range(len(minutelyOhlcv), 0, -1):
                    if dateStr in minutelyOhlcv["trading_point"][i-1]:
                        headMax = i
                        break
                minutelyOhlcvFile = minutelyOhlcv.head(i)
                ...
                action[evalDays-ic] = myStrategy(dailyOhlcvFile,minutelyOhlcvFile,openPricev[evalDays-ic],params)
            ...
            if returnRate > maxRR:
                maxRR = returnRate
                maxParams = params
                tqdm.write("RR = {}  (windowSize: {}, alpha: {}, beta: {})".format(returnRate, windowSize, alpha, beta))
